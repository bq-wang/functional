...
let rec asyncReadTextAndCont (stream: NetworkStream) cont = 
    // unfortunately we need to specific a number of bytes to read
    // this leads to any messages longer than 512 being broken into 
    // different messages
    async { 
            // let buffer = Array.CreateInstance  
            // you can use the following Fsharp function to create array of the following syntax 
            //   Array.CreateInstance(byte, r.InputBlockSize) and new byte[r.InputBlockSize] failed:
            //       http://social.msdn.microsoft.com/Forums/en-US/4978dd54-591d-43b1-92c8-d2b68b2c8ecd/arraycreateinstancebyte-rinputblocksize-and-new-byterinputblocksize-failed
...
type Server() = 
    ...
    let handleClient (connection: TcpClient) =
        ...
        // create a stream write to more eaily write to the client
        ...
        let rec requestAndReadName(stream: NetworkStream) (name: string) = 
            ...
            if clients.ClientExists(name) then
                ...
                sw.WriteLine("ERROR - Name is use already!")
                sw.Flush()
                Async.Start(asyncReadTextAndCont stream requestAndReadName)
            else
    ...
    // create a tcp listener to handle the incoming mesage
    let listener = new TcpListener(IPAddress.Loopback, 4243)
