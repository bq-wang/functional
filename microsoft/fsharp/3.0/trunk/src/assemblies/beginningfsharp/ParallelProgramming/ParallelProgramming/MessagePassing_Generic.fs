...
the ? stands for the optional arguments. 



Some of the key is that: 
1. there is a AsyncReplyChannel<list<'a>>, which is required by the PostAndReply message, be careful of this message, because this will block the thread until it returns. 
*)
// a generic collect that receives a number of post items and 
...
type Collector<'a> (?updatesCount) = 
    ...
    // he API of the collector 
    // add the value to the queue 
    member w.AddValue (x) = mailbox.Post(AddValue(x))

    // get all the values the mail box stores 
    member w.GetValues(x) = mailbox.PostAndReply(fun x -> GetValues x) 
...
let form = 
    ...
    // handle the collectors upates event and use it to post 
    ...
    // when the form closes get the values that were processed
    form.Closed.Add(fun _ ->
        let vals = collector.GetValues()
        MessageBox.Show(sprintf "Value processed: %i" (List.length vals))
        |> ignore
        collector.Stop()) 
    form
